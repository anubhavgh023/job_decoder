...
const HeatMapToolLanguage = ({ heatmapData }: HeatMapProps) => {
  ...
  const matrix = useMemo(() => {
    const initialMatrix: { [language: string]: { [tool: string]: number } } =
      {};
    languages.forEach((lang) => {
      initialMatrix[lang] = {};
      tools.forEach((tool) => {
        initialMatrix[lang][tool] = 0; // Initialize all combinations to 0
      });
    });
  ...
  return (
      <div className="overflow-x-auto rounded-md">
        <table className="table-auto border-collapse w-full">
          <thead>
            <tr>
              <th className="border border-[#4d4d4d] bg-secondary px-2 py-2 text-[#c1ff58] opacity-75">Language / Tool</th>
              {tools.map((tool) => (
                <th
                  key={tool}
              ...
              <tr key={lang}>
                ...
                {tools.map((tool) => (
                  <td
                    ...
                    style={{
                      backgroundColor: `rgba(193, 255, 104, ${
                        matrix[lang][tool] / 4
                      })`,
